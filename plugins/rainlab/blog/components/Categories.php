...
class Categories extends ComponentBase
{
    ...
    protected function loadCategories()
    {
         ...
         */
        $categories = $this->linkCategories($categories);
        $post_num = 3;
        $categories->each(function($category) use ($post_num){
            $categoriIds = [$category['id']];
            if ($category->children) {
                foreach($category->children as $children){
                    $categoriIds[] = $children->id;
                }
            }
            $category['posts'] = Post::join('rainlab_blog_posts_categories', 'post_id', '=', 'rainlab_blog_posts.id')->whereIn('rainlab_blog_posts_categories.category_id', $categoriIds)->orderby('id','desc')->limit($post_num)->get();
        });
        return $categories;
    }
